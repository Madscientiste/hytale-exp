...
/**
 * Loads RCON configuration from server config.json. Reads from the "Mods" section in config.json.
 */
public class RconConfigLoader {

  private static final String CONFIG_FILE = "config.json";
  private static final String MODS_SECTION = "Mods";
  private static final String RCON_PLUGIN_ID = "com.madscientiste.rcon:Rcon";

  /**
   * Load RCON configuration from server config.json. Falls back to defaults if config not found or
   * invalid.
   *
  ...
  /**
   * Find the server's config.json file. Tries multiple locations: 1. Current working directory 2.
   * Parent directory (if running from .server/) 3. Relative to plugin location
   */
  @Nullable
  private static Path findConfigFile() {
    ...
    String[] commonPaths = {
      ".server/" + CONFIG_FILE, "../.server/" + CONFIG_FILE, "server/" + CONFIG_FILE
    };
  ...
  /**
   * Extract RCON configuration from JSON string. Simple parser that looks for "Mods" -> "Rcon"
   * section.
   */
  ...
  private static Map<String, Object> extractRconConfig(String json) {
    try {
      // Simple extraction: find "Mods" -> "com.madscientiste.rcon:Rcon" -> { ... }
      int modsIndex = json.indexOf("\"Mods\"");
      if (modsIndex == -1) {
        return null;
      }

      // Look for the plugin identifier "com.madscientiste.rcon:Rcon"
      int rconIndex = json.indexOf("\"com.madscientiste.rcon:Rcon\"", modsIndex);
      if (rconIndex == -1) {
        return null;
      }
