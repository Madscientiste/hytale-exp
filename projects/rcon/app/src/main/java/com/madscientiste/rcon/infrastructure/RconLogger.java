...
/**
 * Simple logging infrastructure for RCON server. Provides structured logging without external
 * dependencies for MVP.
 */
public class RconLogger {
  private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger("RCON");
  ...
  public void logConnectionClosed(String connectionId, String reason) {
    logger.info("[" + connectionId + "] Connection closed: " + reason + " at " + Instant.now());
  }
  ...
  public void logProtocolViolation(String connectionId, String violation) {
    ...
    if (lastLogged == null
      ...
      logger.warning(
          "[" + connectionId + "] Protocol violation: " + violation + " at " + Instant.now());
