 ...
 * layers.
 * Glue protocol ↔ execution, connection-scoped state, routing messages to
 * handlers.
 */
public class RconApplication implements TransportCallbacks {
    ...
    private void handlePackets(String connectionId, java.util.List<RconPacket> packets) throws Exception {
        ConnectionState state = connectionStates.computeIfAbsent(connectionId, k -> new ConnectionState());
