 ...
 * Tests the complete data flow: Bytes → Transport → Protocol → Application →
 * Response.
 */
public class RconServerIntegrationTest {
    ...
    public void setUp() throws Exception {
        config = new RconConfig("127.0.0.1", 25576, 10, 4096, 5000, 5000, null); // Fixed port for tests, no password
    ...
    public void testMultipleCommandsOnSameConnection() throws Exception {
        ...
        try {
            ...
            for (String command : commands) {
                RconPacket commandPacket = new RconPacket(requestId, RconPacket.SERVERDATA_EXECCOMMAND,
                        "echo " + command);
                client.getOutputStream().write(commandPacket.toBytes());
